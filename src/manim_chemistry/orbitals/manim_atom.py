...
class BohrAtom(VGroup):
    ...
    def calculate_levels(self):
        ...
        return TOTAL_ELECTRONS_PER_LEVEL[next(x for x in list(TOTAL_ELECTRONS_PER_LEVEL.keys()) if x>= self.e)]

    def orbitals_group(self):
        return VGroup(*[Circle(radius=1+i, color=self.orbit_color) for i in range(self.occupied_levels)])

    def nuclei_groups(self):
        protons = [Dot(color=self.proton_color).scale(2).shift(np.random.uniform(-0.25, 0.25, 3)) for i in range(self.p)]
        neutrons = [Dot(color=self.neutron_color).scale(2).shift(np.random.uniform(-0.25, 0.25, 3)) for i in range(self.n)]
...
class Orbital(OpenGLSurface):
    def __init__(
        ...
        **kwargs
        ...
        self.m = m
        self.size = size
        pos = OrbitalPositive(n_value=self.n, l_value=self.l, m_value=self.m, size=self.size).set_color(RED)
        neg = OrbitalNegative(n_value=self.n, l_value=self.l, m_value=self.m, size=self.size).set_color(BLUE)
        self.add(pos, neg)
...
class EnergyLevel(VGroup):
    ...
    def level_lines(self):       
        lines = VGroup(*[Line(0.5*LEFT, 0.5*RIGHT) for i in range(self.number_of_lines[self.level])])
    ...
    def level_index(self):
        buff = 1
        if self.index:
            label = VGroup(*[MathTex(str(self.index) + str(self.level)) for i in range(self.number_of_lines[self.level])])
            buff = 0.75
        else: 
            label = VGroup(*[MathTex(str(self.level)) for i in range(self.number_of_lines[self.level])])
    ...
    def check_number_of_electrons(self):
        if self.electrons > 2 * self.number_of_lines[self.level]:
            raise Exception("You did something wrong, you can not have this much electrons at this level")
...
class Element(VGroup):
    ...
    def make_subelectron_holes(self):
        if self.subelectron_holes:
            return VGroup(*[Hole().move_to(electron.get_center()) for electron in self.electrons])
...
class ElementFrame(VGroup):
    ...
    def __init__(
        ...
        self.frame = Rectangle(height=2, width=1.5, fill_opacity=1).set_color_by_gradient(frame_colors)
...
class NPNTransistor(VGroup):
    def __init__(self, regions_opacity=0.8, **kwargs):
        ...
        self.left_n_region = Rectangle(color=BLUE, fill_opacity=self.regions_opacity, width=3).shift(2*LEFT)
        self.right_n_region = Rectangle(color=BLUE, fill_opacity=self.regions_opacity, width=3).shift(2*RIGHT)
        self.p_region = Rectangle(color=RED, fill_opacity=self.regions_opacity, width=1)
        self.left_electrons = self.make_particles(Electron).move_to(self.left_n_region.get_center())
        self.right_electrons = self.make_particles(Electron).move_to(self.right_n_region.get_center())
        self.holes = self.make_particles(Hole, n_copies=2, horizontal_boundary=[0.32*LEFT, 0.32*RIGHT])
    ...
    def make_particles(self, particle_type, n_copies=5, vertical_boundary=[0.8*UP, 0.8*DOWN], horizontal_boundary=[LEFT, RIGHT]):
        particles = VGroup(*[particle_type().scale(0.5) for i in range(5)])
        particles = arrange_copies_in_rectangle(particles, n_copies, vertical_boundary, horizontal_boundary)
        return particles
        

class BatterySchema(VGroup):
    # TODO: This is not working as intended, rebuild this.
    def __init__(self, mob=Square(), schema_type="horizontal", inverted_terminals=False, **kwargs):
        super().__init__(**kwargs)
    ...
    def horizontal_schema(self):
        ...
        left_bottom_line = Line(left_line.get_bottom(), left_line.get_bottom()+0.45*self.mob.width*RIGHT)
        ...
        right_bottom_line = Line(right_line.get_bottom(), right_line.get_bottom()+0.45*self.mob.width*LEFT)

        self.right_side = VGroup(ending_line, right_line, right_bottom_line)

        # Terminals
        positive_terminal = Line(left_bottom_line.get_right()+0.5*LEFT, left_bottom_line.get_right()+0.5*RIGHT).rotate(PI/2)
        positive_symbol = MathTex("+").next_to(positive_terminal, UP, buff=0.5)
        self.plus_terminal = VGroup(positive_terminal, positive_symbol)

        negative_terminal = Line(right_bottom_line.get_left()+0.25*LEFT, right_bottom_line.get_left()+0.25*RIGHT).rotate(PI/2)
    ...
    def corner_schema(self):
        ...
        # Left side
        starting_line = Line(schema_start, schema_start + 0.2*LEFT)
        left_line = Line(starting_line.get_left(), starting_line.get_left()+(self.mob.height/2+1)*UP)
        left_top_line = Line(left_line.get_top(), left_line.get_top()+0.2*self.mob.width*RIGHT)

        self.l_side = VGroup(starting_line, left_line, left_top_line)

        # Top side
        ending_line = Line(schema_end, schema_end + UP)
        top_line = Line(ending_line.get_top(), ending_line.get_top()+0.2*self.mob.width*LEFT)

        self.top_side = VGroup(ending_line, top_line)

        # Terminals
        positive_terminal = Line(left_top_line.get_right()+0.5*LEFT, left_top_line.get_right()+0.5*RIGHT).rotate(PI/2)
        positive_symbol = MathTex("+").next_to(positive_terminal, UP, buff=0.5)
        self.plus_terminal = VGroup(positive_terminal, positive_symbol)

        negative_terminal = Line(top_line.get_left()+0.25*LEFT, top_line.get_left()+0.25*RIGHT).rotate(PI/2)
        negative_symbol = MathTex("-").next_to(negative_terminal, UP, buff=0.9)
        self.minus_terminal = VGroup(negative_terminal, negative_symbol)
...
class MOSFETTransistor(VGroup):
    ...
    def make_n_region(self, side=LEFT):
        region = Rectangle(
            fill_opacity = 1
        ).set_color(BLUE).scale(0.5).move_to(self.main_board.get_top()).shift(2*side+0.5*DOWN)
    ...
    def add_holes(self):
        ...
        holes = arrange_copies_in_rectangle(holes, 7, vertical_boundary=[1.8*UP, 1.8*DOWN], horizontal_boundary=[3.5*LEFT, 3.5*RIGHT])

        return holes
    

    def add_battery_terminals(self):                
        negative_terminal = Line(0.25*UP, 0.25*DOWN).next_to(self.main_board, UP, buff=1.25).shift(0.25*LEFT)
        positive_terminal = Line(0.5*UP, 0.5*DOWN).next_to(self.main_board, UP, buff=1).shift(0.25*RIGHT)
    ...
    def add_battery(self):
        ...
        return VGroup(self.negative_terminal, self.positive_terminal, self.negative_elbow, self.positive_elbow)
...
class BatterySide(VMobject):
    ...
    def make_points(self):
        ...
        return self.set_points_as_corners([terminal_points, intermediate_point, anchor_points])
...
def add_over_rectangular_surface(mobtype=Dot(), amount=10, height=5, width=5):
    ...
    for i in range(amount):
        group.add(mobtype.copy().shift(
            [np.random.randint(-50*width, 50*width)/100, np.random.randint(-50*height, 50*height)/100, 0]
        ))
...
def arrange_copies_in_rectangle(mobs, n_copies=2, vertical_boundary=[UP, DOWN], horizontal_boundary=[LEFT, RIGHT]):
