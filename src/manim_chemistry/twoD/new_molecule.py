...
class NewMMoleculeObject(VGroup):
    def __init__(
        self,
        atoms_dict: dict,
        bonds_dict: dict,
    ...
    def molecule_from_file(filepath, *args, **kwargs):
        ...
        Returns:
            MMoleculeObject: GraphMolecule from the file
        """
    ...
    def find_atom_position_by_index(self, atom_index: int) -> np.array:
        ...
        Example:
        ```
        molecule = NewMMoleculeObject.from_mol_file("examples/element_files/dimethylpropane.mol")
    ...
    @staticmethod
    def mc_molecule_to_atoms_and_bonds(mc_molecule: MCMolecule) -> Tuple[Dict, Dict]:
        """
...
class NamedMolecule(VGroup):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(molecule_data, NewMMoleculeObject):
            self.molecule = molecule_data

        else:
            self.molecule = NewMMoleculeObject(molecule_data, *args, **kwargs)
    ...
    def from_mol_file(
        ...
        molecule = NewMMoleculeObject.from_mol_file(filename, *args, **kwargs)
    ...
    def from_mol_string(
        ...
        molecule = NewMMoleculeObject.from_mol_string(mol_str, *args, **kwargs)
    ...
    def from_sdf_file(
        ...
        molecules = NewMMoleculeObject.from_sdf_file(filename, *args, **kwargs)
        named_molecules = []
    ...
    def from_sdf_string(
        ...
        molecules = NewMMoleculeObject.from_sdf_string(sdf_str, *args, **kwargs)
        named_molecules = []
        for index, molecule in enumerate(molecules):
            named_molecules.append(
                NamedMolecule(
