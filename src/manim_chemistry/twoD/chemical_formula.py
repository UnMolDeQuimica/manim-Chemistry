...
class ChemicalFormula(MarkupText):
    ...
    """
    def __init__(self, formula: str, metal_color: str=WHITE, non_metal_color: str=WHITE, oxygen_color: str=WHITE, *args, **kwargs):
        ...
        markup = self.make_markup(parsed_formula=self.parsed_formula, metal_color=self.metal_color, non_metal_color=self.non_metal_color, oxygen_color=self.oxygen_color)
    ...
    def make_markup(self, parsed_formula, metal_color: str=WHITE, non_metal_color: str=WHITE, oxygen_color: str=WHITE):
        ...
        for index, atom in parsed_list:
            if atom == "O":
                ...
                markup += self.set_atom_color(atom, parsed_formula[atom], non_metal_color)
...
class NamedFormula(VGroup):
    def __init__(self, metal_name: str, non_metal_name: str, formula: str, metal_color: str=WHITE, non_metal_color: str=WHITE, oxygen_color: str=WHITE, font: str = "", spanish_structure = False, buff=DEFAULT_MOBJECT_TO_MOBJECT_BUFFER, direction=DOWN, *args, **kwargs):
        ...
        self.font = font
        chemical_formula = ChemicalFormula(formula=self.formula, metal_color=self.metal_color, non_metal_color=self.non_metal_color, oxygen_color=self.oxygen_color, *args, **kwargs)
        self.add(chemical_formula)
        self.name = self.make_name().next_to(chemical_formula, direction=direction, buff=buff)
        self.add(self.name)
    ...
    def make_name(self):
        ...
        if self.non_metal_name:
            non_metal_text = f"<span fgcolor='{self.non_metal_color}'>{self.non_metal_name}</span>"

        if self.spanish_structure:
            return MarkupText(" ".join(filter(None, [non_metal_text, metal_text])), font=self.font)
            
        return MarkupText(" ".join(filter(None, [metal_text, non_metal_text])), font=self.font)
...
class ComplexFormula(MarkupText):
    ...
    def __init__(self, formula_dict: dict, *args, **kwargs):
        ...
        for formula, color in formula_dict.items():
            markup += f"<span fgcolor='{color}'>{self.make_formula_structure(formula)}</span>"
...
class NamedComplexFormula(VGroup):
    ...
    """
    def __init__(self, name_dict: dict, formula_dict: dict, direction=DOWN, buff=DEFAULT_MOBJECT_TO_MOBJECT_BUFFER, *args, **kwargs):
