...
class GraphMolecule(Graph):
    ...
    def depth_first_search(self, graph: nx.Graph, atom: int, visited_atoms: set, connected_atoms: list) -> list:
        """
    ...
    def get_connected_atoms(self, from_atom_index: int, to_atom_index: int) -> list:
        ...
        if not self._graph.has_edge(from_atom_index, to_atom_index):
            raise Exception(f"There is no bond between atoms {from_atom_index} and {to_atom_index}")
    ...
    def get_connected_atoms_v_group(self, from_atom_index: int, to_atom_index: int) -> list:
        connected_atoms = self.get_connected_atoms(from_atom_index=from_atom_index, to_atom_index=to_atom_index)
        return self.get_atoms_vgroup_from_index(connected_atoms)
    
    def get_bonds_from_atoms_indices(self, connected_atoms: list, excluded_atom: int = 0) -> list:
        """
    ...
    def get_connected_atoms_and_bonds_group_from_index(self, connected_atoms: list, excluded_atom: int=0):
        
        return VGroup(
            self.get_atoms_vgroup_from_index(atoms_indices=connected_atoms),
            self.get_bonds_vgroup_from_index(connected_atoms=connected_atoms, excluded_atom=excluded_atom)
        )
        
    def get_connected_atoms_and_bonds(self, from_atom: int, to_atom: int) -> VGroup:
        connected_atoms = self.get_connected_atoms(from_atom_index=from_atom, to_atom_index=to_atom)
        return self.get_connected_atoms_and_bonds_group_from_index(connected_atoms=connected_atoms, excluded_atom=from_atom)