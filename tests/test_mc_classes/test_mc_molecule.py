...
mol_files = [f"{os.path.join(base_files_path, 'mol_files', file)}.mol" for file in files_names]
sdf_files = [f"{os.path.join(base_files_path, 'sdf_files', file)}.sdf" for file in files_names]
json_files = [f"{os.path.join(base_files_path, 'json_files', file)}.json" for file in files_names]
asnt_files = [f"{os.path.join(base_files_path, 'asnt_files', file)}.asnt" for file in files_names if file != "heme_2d"]
xml_files = [f"{os.path.join(base_files_path, 'xml_files', file)}.xml" for file in files_names]
...
class TestMCMolecule:
    ...
    def test_construct_from_file_removing_hydrogens(self, file):
        ...
        without_hydrogens = MCMolecule.construct_from_file(file, ignore_all_hydrogens=True)

        assert len(with_hydrogens.atoms) >= len(without_carbon_hydrogens.atoms) >= len(without_hydrogens.atoms)
        assert len(with_hydrogens.bonds) >= len(without_carbon_hydrogens.bonds) >= len(without_hydrogens.bonds)
    ...
    def test_reindex_molecule_atoms(self, file):
        ...
        for index, bond in enumerate(molecule_b.bonds, start=1):
            if bond.from_atom.molecule_index == molecule_b_atom.molecule_index or bond.to_atom.molecule_index == molecule_b_atom.molecule_index:
