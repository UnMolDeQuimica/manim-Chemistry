...
from manim_chemistry.utils import SDFParser
from .sdf_parser_fixtures import BaseTestSDFParser
...
class TestSDFParser(BaseTestSDFParser):
    ...
    def test_read_files(
        ...
        assert acetone_2d_file_data == SDFParser.read_file(self.acetone_2d)

        # Test read acetone 3D file
        assert acetone_3d_file_data == SDFParser.read_file(self.acetone_3d)

        # Test read 2D morphine file
        assert morphine_2d_file_data == SDFParser.read_file(self.morphine_2d)

        # Test read morphine 3D file
        assert morphine_3d_file_data == SDFParser.read_file(self.morphine_3d)

        # Test read heme 2d
        assert heme_2d_file_data == SDFParser.read_file(self.heme_2d)

        # Test read heme 3d
        assert heme_3d_file_data == SDFParser.read_file(self.heme_3d)
    ...
    def test_parse_files(
        ...
        self.compare_read_molecule_and_known_values(
            SDFParser(self.acetone_2d), acetone_2d_molecule_data
        )

        # Test parse 3D acetone file
        self.compare_read_molecule_and_known_values(
            SDFParser(self.acetone_3d),
            acetone_3d_molecule_data,
        )
